...
module.exports = function (app, nconf, io, zio, topic_in, topic_out) {
  ...
  var emitChat = function (socket, chat, zio, topic_out) {
    var statmsg = topic_out.concat('|', JSON.stringify({ epoch_ms: (new Date).getTime(), fingerprint: chat.value.fingerprint }));
    zio.send(statmsg);
  ...
  var addChat = function (message, picture, fingerprint, userId, ip, next) {
    publico.addChat(message.slice(0, 250), {
      ...
      if (err) {
        ...
        try {
          var statmsg = topic_in.concat('|', JSON.stringify({ epoch_ms: (new Date).getTime(), fingerprint: fingerprint }));
          zio.send(statmsg);
